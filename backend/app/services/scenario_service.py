...
def run_scenario(name: str, target: str) -> dict[str, Any]:
    ...
    try:
        ...
        busybox_path = _ensure_busybox_helper(client, container)
        kill_shell_prefix, script_shell_prefix = _resolve_shell(container, busybox_path)
        _ensure_busybox_shims(container, busybox_path, kill_shell_prefix)

        _put_file(container, script_path, script_dest)
        if not _exec_ok(container, ["chmod", "+x", script_dest]):
            _exec_ok(container, [busybox_path, "chmod", "+x", script_dest])

        kill_cmd = (
            f"(command -v pkill >/dev/null 2>&1 && pkill -f {shlex.quote(script_dest)}) "
            ">/dev/null 2>&1 || true"
        )
        container.exec_run(kill_shell_prefix + [kill_cmd])
        container.exec_run(
            script_shell_prefix + [script_dest],
            detach=True,
            environment={
                "BUSYBOX_BIN": busybox_path,
                "PATH": f"{_BUSYBOX_SHIM_DIR}:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
            },
        )

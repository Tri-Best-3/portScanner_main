...
ensure_busybox_cache() {
  ...
  if mkdir "$BUSYBOX_CACHE_LOCK" 2>/dev/null; then
    ...
    if ! docker run --rm "$BUSYBOX_IMAGE" sh -lc "apk add --no-cache busybox-static >/dev/null 2>&1 && cat /bin/busybox.static" > "$helper_tmp"; then
      rm -f "$helper_tmp"
...
prepare_busybox_helper() {
  ...
  if ! docker cp "$BUSYBOX_CACHE" "$CONTAINER_NAME:$BUSYBOX_DEST" >/dev/null 2>&1; then
    return 1
  fi

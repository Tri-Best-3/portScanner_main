...
def get_scenarios(url):
    try:
        res = requests.get(f"{url}/api/v1/scenarios", timeout=3)
        if res.status_code != 200:
            return [], f"scenario request failed ({res.status_code})"
        payload = res.json()
...
def get_verifications(url, scan_id):
    try:
        res = requests.get(f"{url}/api/v1/verifications/{scan_id}", timeout=8)
        if res.status_code != 200:
            return [], f"verification request failed ({res.status_code})"
        payload = res.json()
        items = payload.get("items", []) if isinstance(payload, dict) else []
...
def render_dashboard(data, backend_url):
    if not data:
        return
    ...
    t_ai, t_ports, t_vulns, t_verify, t_logs, t_json = st.tabs(
        ["🤖 AI 브리핑", "🌐 포트 현황", "🚨 취약점 상세", "✅ 검증 결과", "📝 Raw 로그", "🧾 JSON"]
    )
    ...
    with t_verify:
        if not scan_id:
            st.info("scan_id가 없어 검증 결과를 조회할 수 없습니다.")
        else:
            verification_items, verification_error = get_verifications(backend_url, scan_id)
            if verification_error:
                st.warning(f"검증 결과 조회 실패: {verification_error}")
            elif not verification_items:
                st.info("자동 검증 결과가 없습니다.")
            else:
                verification_rows = [
                    {
                        ...
                        "created_at": convert_to_kst(item.get("created_at")),
                    }
...
scenario_items, scenario_error = get_scenarios(backend_url)
...
with st.expander("🚀 스캔 실행", expanded=True):
    ...
    scenario_options = ["(none)"] + [item["name"] for item in scenario_items]
    if st.session_state["selected_scenario_name"] not in scenario_options:
        st.session_state["selected_scenario_name"] = "(none)"

    selected_scenario_name = st.selectbox(
        "Scenario",
        scenario_options,
        index=scenario_options.index(st.session_state["selected_scenario_name"]),
        help="Loaded from GET /api/v1/scenarios",
    )
    st.session_state["selected_scenario_name"] = selected_scenario_name
    selected_scenario = None if selected_scenario_name == "(none)" else selected_scenario_name

    if scenario_error:
        st.caption(f"Scenario list fetch failed: {scenario_error}")
    elif selected_scenario:
        scenario_meta = next((item for item in scenario_items if item["name"] == selected_scenario), None)
        if scenario_meta:
            info_chunks = []
            if scenario_meta.get("target_hint"):
                info_chunks.append(f"target_hint={scenario_meta['target_hint']}")
            if scenario_meta.get("description"):
                info_chunks.append(scenario_meta["description"])
            if info_chunks:
        ...
        if not targets_to_run:
            ...
            for target in targets_to_run:
                with st.spinner(f"[{target}] 분석 중..."):
                    try:
                        workflow_request = {"target": target, "profile": profile}
                        if selected_scenario:
                            workflow_request["scenario"] = selected_scenario
                        res = requests.post(
...
if st.session_state["last_scan_data"]:
    render_dashboard(st.session_state["last_scan_data"], backend_url)
